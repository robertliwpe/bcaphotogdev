...
if (!function_exists('creative_lab_system_fonts_list')) {
	function creative_lab_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('creative_lab_text_decoration_list')) {
	function creative_lab_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'creative-lab'), 
			'underline' => esc_html__('underline', 'creative-lab'), 
			'overline' => esc_html__('overline', 'creative-lab'), 
			'line-through' => esc_html__('line-through', 'creative-lab'), 
		);
...
if (!function_exists('creative_lab_custom_color_schemes_list')) {
	function creative_lab_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'creative-lab'), 
			'second' => esc_html__('Custom 2', 'creative-lab'), 
			'third' => esc_html__('Custom 3', 'creative-lab') 
		);
		
		
		return apply_filters('creative_lab_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('creative_lab_locate_template')) {
	function creative_lab_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_creative-lab_theme_style') ? get_option('cmsmasters_creative-lab_theme_style') : ''));
...
if (!function_exists('creative_lab_all_theme_styles')) {
	function creative_lab_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('creative_lab_all_color_schemes_list')) {
	function creative_lab_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'creative-lab'), 
			'header' => 		esc_html__('Header', 'creative-lab'), 
			'navigation' => 	esc_html__('Navigation', 'creative-lab'), 
			'header_top' => 	esc_html__('Header Top', 'creative-lab'), 
			'footer' => 		esc_html__('Footer', 'creative-lab') 
		);
		
		
		$out = array_merge($list, creative_lab_custom_color_schemes_list());
...
if (!function_exists('creative_lab_load_theme_textdomain')) {
	function creative_lab_load_theme_textdomain() {
		load_theme_textdomain('creative-lab', CMSMASTERS_VARS . '/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'creative_lab_load_theme_textdomain')) {
	add_action('after_setup_theme', 'creative_lab_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('creative_lab_theme_activation')) {
	function creative_lab_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'creative-lab') {
			add_option('cmsmasters_active_theme', 'creative-lab', '', 'yes');
...
add_action('after_switch_theme', 'creative_lab_theme_activation');



// Framework Deactivation
if (!function_exists('creative_lab_theme_deactivation')) {
	function creative_lab_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'creative_lab_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('creative_lab_plugin_activation')) {
	function creative_lab_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'creative_lab_plugin_activation', 10, 2);


if (!function_exists('creative_lab_plugin_activation_regenerate')) {
	function creative_lab_plugin_activation_regenerate() {
...
add_action('init', 'creative_lab_plugin_activation_regenerate');



function creative_lab_run_reinit_import_options($post_id, $key, $value) {
...
add_action('import_post_meta', 'creative_lab_run_reinit_import_options', 10, 3);
