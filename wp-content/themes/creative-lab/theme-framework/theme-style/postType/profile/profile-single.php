...
if (
	$cmsmasters_option['creative-lab' . '_profile_post_cat'] || 
	$cmsmasters_option['creative-lab' . '_profile_post_comment'] || 
	(
	...
	echo '<div class="profile_cont' . (($profile_sidebar == 'true') ? ' with_sidebar' : '') . '">';
		...
		if ($cmsmasters_profile_title == 'true') {
			echo '<header class="cmsmasters_profile_header entry-header">';
				creative_lab_profile_title_nolink(get_the_ID(), 'h3', $cmsmasters_profile_subtitle, 'h6');
			echo '</header>';
		}
		...
		if ($cmsmasters_profile_sharing_box == 'true') {
			creative_lab_sharing_box(esc_html__('Share:', 'creative-lab'), 'h6');
		}
		...
		echo '<div class="profile_sidebar">';
			...
			if ($profile_details == 'true') {
				echo '<div class="profile_details entry-meta">' . 
					...
					creative_lab_get_profile_likes('post');
					
					creative_lab_get_profile_comments('post');
					
					creative_lab_get_profile_features('details', $cmsmasters_profile_features, false, 'h5', true);
					
					creative_lab_get_profile_category(get_the_ID(), 'pl-categs', 'post');
					
				echo '</div>';
			}
			
			
			creative_lab_get_profile_features('features', $cmsmasters_profile_features_one, $cmsmasters_profile_features_one_title, 'h5', true);
			
			creative_lab_get_profile_features('features', $cmsmasters_profile_features_two, $cmsmasters_profile_features_two_title, 'h5', true);
			
			creative_lab_get_profile_features('features', $cmsmasters_profile_features_three, $cmsmasters_profile_features_three_title, 'h5', true);
			
			
			creative_lab_profile_open_social_icons($cmsmasters_profile_social, get_the_ID(), esc_html__('Socials', 'creative-lab'), 'h5');
...
if ($cmsmasters_option['creative-lab' . '_profile_post_nav_box']) {
	$order_cat = (isset($cmsmasters_option['creative-lab' . '_profile_post_nav_order_cat']) ? $cmsmasters_option['creative-lab' . '_profile_post_nav_order_cat'] : false);
	
	creative_lab_prev_next_posts('Previous Profile', 'Next Profile', $order_cat, 'pl-categs');
}
