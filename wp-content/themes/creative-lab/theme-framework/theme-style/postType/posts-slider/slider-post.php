...
$excerpt = (in_array('excerpt', $cmsmasters_metadata) && creative_lab_slider_post_check_exc_cont('post')) ? true : false;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_slider_post'); ?>>
	...
	<?php
		if (has_post_thumbnail()) {
			creative_lab_thumb_rollover(get_the_ID(), 'cmsmasters-blog-masonry-thumb', false, false, false, false, false, false, false, false, true, false, false);
		}
			...
			echo '<div class="cmsmasters_slider_post_inner">';
				...
				if ($categories || $author) {
					echo '<div class="cmsmasters_slider_post_cont_info entry-meta">';
						...
						$categories ? creative_lab_get_slider_post_category(get_the_ID(), 'category', 'post') : '';
						
						$author ? creative_lab_get_slider_post_author('post') : '';
						
					echo '</div>';
				}
				
				
				$title ? creative_lab_slider_post_heading(get_the_ID(), 'post', 'h3') : '';
				
				$excerpt ? creative_lab_slider_post_exc_cont('post') : '';
				
				$more ? creative_lab_slider_post_more(get_the_ID()) : '';
				
				
				if ($date || $likes || $comments) {
					echo '<footer class="cmsmasters_slider_post_footer entry-meta">';
					
						$date ? creative_lab_get_slider_post_date('post') : '';
						
						
						if ($likes || $comments) {
							echo '<div class="cmsmasters_slider_post_meta_info">';
							
								$likes ? creative_lab_slider_post_like('post') : '';
								
								$comments ? creative_lab_get_slider_post_comments('post') : '';
