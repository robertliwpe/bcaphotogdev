...
if (
	$cmsmasters_option['creative-lab' . '_portfolio_project_like'] || 
	$cmsmasters_option['creative-lab' . '_portfolio_project_date'] || 
	$cmsmasters_option['creative-lab' . '_portfolio_project_cat'] || 
	$cmsmasters_option['creative-lab' . '_portfolio_project_comment'] || 
	$cmsmasters_option['creative-lab' . '_portfolio_project_author'] || 
	$cmsmasters_option['creative-lab' . '_portfolio_project_tag'] || 
	...
	echo '<div class="project_content' . (($project_sidebar == 'true') ? ' with_sidebar' : '') . '">';
		if ($cmsmasters_project_title == 'true') {
			echo '<header class="cmsmasters_project_header entry-header">';
				creative_lab_project_title_nolink(get_the_ID(), 'h3');
			echo '</header>';
		}
		...
		if ($cmsmasters_post_format == 'gallery') {
			...
			creative_lab_post_type_gallery(get_the_ID(), $cmsmasters_project_images, $cmsmasters_project_columns, 'cmsmasters-project-thumb', 'cmsmasters-full-masonry-thumb');
			...
			creative_lab_post_type_video(get_the_ID(), $cmsmasters_project_video_type, $cmsmasters_project_video_link, $cmsmasters_project_video_links, 'cmsmasters-full-masonry-thumb');
			...
			if ($cmsmasters_project_image_show == 'true' && $cmsmasters_project_images[0] == '') {
				...
				creative_lab_post_type_slider(get_the_ID(), $cmsmasters_project_images, 'cmsmasters-full-masonry-thumb');
			}
		}
		...
		echo '<div class="project_sidebar">';
			...
			if ($project_details == 'true') {
				echo '<div class="project_details entry-meta">' . 
					...
					creative_lab_get_project_likes('post');
					
					creative_lab_get_project_comments('post');
					
					creative_lab_get_project_author('post');
					
					creative_lab_get_project_date('post');
					
					creative_lab_get_project_category(get_the_ID(), 'pj-categs', 'post');
					
					creative_lab_get_project_tags(get_the_ID(), 'pj-tags');
					
					creative_lab_get_project_features('details', $cmsmasters_project_features, false, 'h3', true);
					
					creative_lab_project_link($cmsmasters_project_link_text, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
					
				echo '</div>';
			}
			
			
			creative_lab_get_project_features('features', $cmsmasters_project_features_one, $cmsmasters_project_features_one_title, 'h5', true);
			
			creative_lab_get_project_features('features', $cmsmasters_project_features_two, $cmsmasters_project_features_two_title, 'h5', true);
			
			creative_lab_get_project_features('features', $cmsmasters_project_features_three, $cmsmasters_project_features_three_title, 'h5', true);
			
			
			if ($cmsmasters_project_sharing_box == 'true') {
				creative_lab_sharing_box(esc_html__('Shared', 'creative-lab'), 'h5');
			}
...
if ($cmsmasters_option['creative-lab' . '_portfolio_project_nav_box']) {
	$order_cat = (isset($cmsmasters_option['creative-lab' . '_portfolio_project_nav_order_cat']) ? $cmsmasters_option['creative-lab' . '_portfolio_project_nav_order_cat'] : false);
	
	creative_lab_prev_next_posts('Previous Project', 'Next Project', $order_cat, 'pj-categs');
}


if ($cmsmasters_project_author_box == 'true') {
	creative_lab_author_box(esc_html__('About author', 'creative-lab'), 'h2', 'h4');
}
...
if ($cmsmasters_project_more_posts != 'hide') {
	creative_lab_related( 
		'h2', 
		esc_html__('More projects', 'creative-lab'), 
		esc_html__('No projects found', 'creative-lab'), 
		$cmsmasters_project_more_posts, 
		$tgsarray, 
		$cmsmasters_option['creative-lab' . '_portfolio_more_projects_count'], 
		$cmsmasters_option['creative-lab' . '_portfolio_more_projects_pause'], 
		'project', 
		'pj-tags' 
	);
}
