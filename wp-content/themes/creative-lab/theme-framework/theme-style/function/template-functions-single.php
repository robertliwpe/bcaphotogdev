...
function creative_lab_prev_next_posts($prev_sub = '', $next_sub = '', $order_cat = false, $tax_name = 'category') {
	$cmsmasters_post_type = get_post_type();
	
	$published_posts = wp_count_posts($cmsmasters_post_type)->publish;
	
	
	$prev_sub = ($prev_sub != '' ? $prev_sub : esc_html__('previous', 'creative-lab'));
	$next_sub = ($next_sub != '' ? $next_sub : esc_html__('next', 'creative-lab'));
...
function creative_lab_sharing_box($title_box = false, $tag = 'h3') {
...
function creative_lab_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a class="about_author_url " href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'creative-lab') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function creative_lab_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
		...
		if ($r_query->have_posts()) {
			echo "<aside class=\"cmsmasters_single_slider" . (($type == 'post') ? ' cmsmasters_type_post' : ' cmsmasters_type_project') . "\">" . 
				"<" . esc_html($tag) . " class=\"cmsmasters_single_slider_title\">" . 
					($title != '' ? esc_html($title) : esc_html__('More items', 'creative-lab')) . 
				"</" . esc_html($tag) . ">" . 
							...
							echo "<div id=\"post-"; the_ID(); echo"\" class=\"cmsmasters_owl_slider_item cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">
									<div class=\"cmsmasters_single_slider_item_inner\">";
										...
										if ($type == 'project') {
											creative_lab_get_projects_color(get_the_ID());
											
											creative_lab_thumb(get_the_ID(), 'cmsmasters-project-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
											...
											creative_lab_get_post_category(get_the_ID(), 'category');
											
											echo "<h5 class=\"cmsmasters_single_slider_item_title\">
												<a href=\"" . esc_url(get_permalink()) . "\">" . cmsmasters_title(get_the_ID(), false) . "</a>
											</h5>";
											
											creative_lab_post_exc_cont();
...
function creative_lab_author_avatar($template_type = 'page') {
...
function creative_lab_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (pings_open($id) && sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'creative-lab') . '</' . esc_html($tag) . '>' . "\n" .
